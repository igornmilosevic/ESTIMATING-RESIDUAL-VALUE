...
for col in train.columns:
    ...
    if col not in ['SalesID','missing','MachineHoursCurrentMeter_missing', 'MachineHoursCurrentMeter', 'SalePrice', 'cluster'
        , 'month', 'saledate', 'age', 'age_imputed', 'machine_count', 'MachineID', 'salemonth', 'CPI', 'INDPROD', 'GDP', 'PPI']:
        print(col)
...
# train.to_csv("NN_model_dataset_" + str(ModeID) + "_.csv", index=False)
...
valid.drop(['SalesID','SalePrice', 'saledate', 'month', 'MachineID'], axis=1, inplace=True)
train.drop(['SalesID','SalePrice', 'saledate', 'month', 'MachineID'], axis=1, inplace=True)
...
for ind_val in ind_vals:
    for machine_val in machine_vals:
        ...
        valid_copy = scale.transform(valid_copy)
        pred = model.predict(valid_copy)

        valid_cpy = pd.DataFrame()
        valid_cpy['pred'] = np.exp(pred)
        valid_cpy['actual'] = np.exp(y_test)
        ...
        print("Correlation Coeff test set: ", valid_cpy['actual'].corr(valid_cpy['pred']))

        valid_cpy.to_csv("Validation_set_2_4_predictions_NN_" + str(ind_val) + "_" + str(machine_val) + "_.csv", index=False)